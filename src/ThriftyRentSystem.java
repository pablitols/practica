...
class  ThriftyRentSystem{
	...
	private void add(Scanner scan)
    {
		...
		if(vehicleType.equalsIgnoreCase("van"))
		{
			...
			FechaHora Lastmain= new FechaHora(Integer.parseInt(dateSplit[0]),Integer.parseInt(dateSplit[1]),Integer.parseInt(dateSplit[2]));
			if(i<50){
				Vehicle newVehicle= new Van(vehicleID,year,make,model,0,new VehicleType(seats,Lastmain), year);
	...
	FechaHora rentDate= new FechaHora(Integer.parseInt(dates[0]),Integer.parseInt(dates[1]),Integer.parseInt(dates[2]));
		...
		if(this.cars[0]!=null && id.contains("C_"))
        {
            ...
            for(int i=0;this.cars[i]!=null;i++){
                ...
                if ((this.cars[i].getVehicleId()).equals(id))
                {
				   ...
				   String date = sc.next();
				   String dates[] = date.split("/");
				   FechaHora returnDate= new FechaHora(Integer.parseInt(dates[0]),Integer.parseInt(dates[1]),Integer.parseInt(dates[2]));
		...
		if(this.vans[0]!=null && id.contains("V_"))
        {
            ...
            for(int i=0;this.vans[i]!=null;i++){
                if ((this.vans[i].getVehicleId()).equals(id))
                {
				    ...
				    String date = sc.next();
					String dates[] = date.split("/");
					FechaHora maintenanceDate= new FechaHora(Integer.parseInt(dates[0]),Integer.parseInt(dates[1]),Integer.parseInt(dates[2]));
